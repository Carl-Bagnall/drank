...
- **The iteration count is 100,000, and that is a ceiling rather than a choice.** Cloudflare's WebCrypto rejects PBKDF2 above 100,000 iterations — a platform limit, not a plan limit, so a paid plan does not raise it. OWASP recommends 600,000 for PBKDF2-HMAC-SHA-256, so this sits meaningfully below current guidance. Closing that gap means changing algorithm (bcrypt or Argon2 via WASM), not turning a number up. Each hash records the count it was made with, so a future change will not invalidate existing passwords.
